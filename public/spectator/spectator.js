...
const setStyledInnerHTML = (element = null, text = '') => {
	if (element == null) element = document.getElementById('container');
	element.innerHTML = text.replace(/\([^\)]*?\)/g, '<span class="multiplier">$&</span>');
};
...
const startConnection = () => {
	...
	ws.onmessage = (wsMessage) => {
		...
		if (msg.service === 'spectator') {
			...
			if (Object.entries(lastData).toString() === Object.entries(nextData).toString()) return;

			setTimeout(() => {
				setInnerHTML(program, nextData.program ? nextData.program : '');
				setStyledInnerHTML(currentStudent, nextData.current ? nextData.current : '');
